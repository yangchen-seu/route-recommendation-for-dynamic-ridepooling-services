...
if True:
    ...
    # with open ('tmp/shortest_paths_and_distances.csv','rb') as f:
    #     shortest_paths_and_distances= pd.read_csv(f)
...
def search_matching_pairs(OD_idx1, OD_idx2, OD1, OD2, L1, L2, distance_between_dest, distance_from_seeker_origin_to_taker_dest):
    ...
    if nearest_G.has_node(OD2[0]): # 附近有给定OD2的起点 在taker起点就匹配上了
    ...
    path_dic = {'shortest_path':OD_dict[OD_idx2][4],'highest_potential_path':OD_dict[OD_idx2][5]}
...
#     m = pd.DataFrame(result, columns=["seeker_id", "taker_id","taker_route_type", "taker_route_path",  "link_idx", "preference", "ride_seeker", "ride_taker", "detour_seeker", "detour_taker", "shared",'destination'])
