...
class Predict():
    ...
    # def run(self, pi, theta ) -> None:
        ...
        if isPrestored == 'n':
            ...
            for seeker_id, OD in OD_dict.items():
                seekers[seeker_id] = {"lambda": OD[2], "p_seeker": 0.1,'lambda_1':0,'lambda_2':0,'lambda_3':0,'lambda_4':0,'max_length':4,'mu_w':0.01 } # params['K']/len(OD_dict)
            ...
            for taker_id in OD_dict.keys():
                ...
                # 获取 taker 的所有路径
                paths = {
                    'shortest_path': OD_dict[taker_id][4], 
                    'highest_potential_path': OD_dict[taker_id][5]
                }
        ...
        # while iter_num < 1: 
            ...
            for seeker_id in seekers.keys():
                ...
                if seekers[seeker_id]["p_seeker"] == 1:
                    ...
                    for link_idx, link in taker.items(): # loop for link a
                        ...
                        if mu_ == 0 :
                            ...
                            if lambda_ >= mu_:
                                ...
                                # update p_taker, rho_taker 
                                link["p_taker"] =  1 - np.exp(-(lambda_-mu_)*link["tau_bar"] )
        ...
        for seeker_id, takers_of_seeker in matches.items():
            ...
            for taker in takers_of_seeker:
                L1 = OD_dict[seeker_id][3]
                L2 = OD_dict[taker["taker_id"]][3]
        ...
        for seeker_id in seekers.keys():
            ...
            for path in ['shortest_path','highest_potential_path']:
                seekers[seeker_id]["ride_distance_for_taker"] += takers[seeker_id][path][len(takers[seeker_id]) - 1]["lambda_taker"] * (1 - takers[seeker_id][path][len(takers[seeker_id]) - 1]["p_taker"]) / lambda_become_taker * L
    ...
    "ride_distance_for_seeker", "detour_distance_for_seeker", "shared_distance_for_seeker", "saved_distance_for_seeker",'shortest_path_prob','cancel_rate','p_seeker']]
        ...
        result.to_csv("result/predict_result_optimized.csv")
