...
def calculate_dijkstra_path(OD_infor):
    ...
    """
    OD_id, OD = OD_infor
    (length, path) = nx.single_source_dijkstra(G, source=OD['start'], target=OD['destination'], weight='weight')
    node_list = path
    result = list()
...
def generate_ego_graph(OD_infor):
    ...
    """
    OD_id, OD = OD_infor
    distance, ego_graph = nx.generators.ego_graph(G, OD['start'], radius=params['search_radius'], distance="weight")
...
if __name__ == '__main__':
    ...
    f = open(params['shortest_path_pickle'], 'wb')
    ...
    # ---------- dump the ego graphs ----------
    f = open(params['ego_graph_pickle'], 'wb')
    pickle.dump(dict(result), f)
    f.close()
