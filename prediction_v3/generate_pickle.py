...
def find_max_weight_path_dag_with_length(G, source, target):
    ...
    while current is not None:
        if predecessor[current] is not None:  # 累加边的权重
            ...
            total_length_list.append(G[predecessor[current]][current]['weight'])
        path.append(current)
        current = predecessor[current]
    path.reverse()
    
    return path, max_weight[target], total_weight_length , total_length_list
...
for i in bar.iter(range(OD_num)):
    ...
    shortest_path_length_lis =  [G[u][v]["weight"] for u, v in zip(shortest_path[:-1], shortest_path[1:])]
    ...
    high_potential_path, max_passengers, path_weight_length ,total_length_list = find_max_weight_path_dag_with_length(D_, star, dest)


    OD_dict[int(OD_data.loc[i].loc["OD_id"])] = {
        "start": star,
        "destination": dest,
        "lambda": OD_data.loc[i].loc["lambda"],
        "solo_dist": shortest_length,  # 最短路径长度
        "shortest_path": shortest_path,  # 最短路径
        "shortest_path_link_length_list": shortest_path_length_lis,  # 最短路径的每段距离
        "high_potential_path": high_potential_path,  # 高潜力路径
        "max_matching_passengers": max_passengers,  # 最大匹配乘客数
        "high_potential_path_length_total": path_weight_length,  # 高潜力路径总长
        "high_potential_path_length_list": total_length_list,  # 高潜力路径的每段距离
        "prob_shortest_path": 1,  # 选择最短路径的概率
    }


    
    
f = open(settings.params['OD_pickle_file'], 'wb')
...
for key in node_dict.keys():
    ...
    for OD_id in OD_dict.keys():
        if OD_dict[OD_id]['destination'] == key:
            lis.append(OD_id)
    node_dict[key][2] = lis

f = open(settings.params['node_pickle_file'], 'wb')
pickle.dump(node_dict, f)
f.close()
